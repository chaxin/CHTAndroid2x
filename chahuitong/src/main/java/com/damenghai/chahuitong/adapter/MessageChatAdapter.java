...
public class MessageChatAdapter extends CommonAdapter<BmobMsg> {
	//8种Item的类型
	//文本
    ...
    private ViewHolder createViewByType(int position, View convertView, ViewGroup parent) {
        ...
        if(type==BmobConfig.TYPE_IMAGE){//图片类型
            return getItemViewType(position) == TYPE_RECEIVER_IMAGE ?
                    ViewHolder.get(convertView, mContext, R.layout.item_chat_received_image, parent, position)
                    :
                    ViewHolder.get(convertView, mContext, R.layout.item_chat_sent_image, parent, position);
        }else if(type==BmobConfig.TYPE_LOCATION){//位置类型
            return getItemViewType(position) == TYPE_RECEIVER_LOCATION ?
                    ViewHolder.get(convertView, mContext, R.layout.item_chat_received_location, parent, position)
                    :
                    ViewHolder.get(convertView, mContext, R.layout.item_chat_sent_location, parent, position);
        }else if(type==BmobConfig.TYPE_VOICE){//语音类型
            return getItemViewType(position) == TYPE_RECEIVER_VOICE ?
                    ViewHolder.get(convertView, mContext, R.layout.item_chat_received_voice, parent, position)
                    :
                    ViewHolder.get(convertView, mContext, R.layout.item_chat_sent_voice, parent, position);
        }else{//剩下默认的都是文本
            ...
            case BmobConfig.TYPE_IMAGE://图片类
                try {
                    if (!TextUtils.isEmpty(text)) {//发送成功之后存储的图片类型的content和接收到的是不一样的
                        L.d("text: " + text + ", type: " + getItemViewType(position));
...
//			如果是发送的图片的话，因为开始发送存储的地址是本地地址，发送成功之后存储的是本地地址+"&"+网络地址，因此需要判断下
            ...
            L.d("ImageUrl:" + text + ", showUrl: " + showUrl);
            //为了方便每次都是取本地图片显示
