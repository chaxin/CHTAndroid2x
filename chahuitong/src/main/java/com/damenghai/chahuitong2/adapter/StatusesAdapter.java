...
import com.damenghai.chahuitong2.ui.activity.PersonalActivity;
...
public class StatusesAdapter extends BaseListAdapter<Status> {
    ...
    public void convert(final ViewHolder holder, final Status status) {
        ...
        if(mShowUser) {
            ...
            if(status.getMemberInfo() != null) {
                ...
                bu.cancel();
                holder.setText(R.id.status_user, "")
                        .cancelBitmapUtil(R.id.status_avatar);
            }
    ...
    protected void setImages(ViewHolder holder, final Status status) {
        ...
        if(picUrls != null && picUrls.size() > 1) {
            ...
            bitmapUtils.display(iv_image, picUrls.get(0).getBmiddle_pic(), ImageConfigHelper.getGrayConfig(mContext));
