...
public class ViewHolder {
    ...
    public ViewHolder setChecked(int resId, boolean checked) {
	...
	public ViewHolder displayImageUri(int viewId, Uri uri) {
		...
		utils.display(iv, (uri.toString().contains("file")) ? uri.toString() : ImageUtils.getRealPathFromURI(uri, mContext), ImageConfigHelper.getGrayConfig(mContext));
		return this;
	}
	...
	public ViewHolder loadDefaultImage(int viewId, String url) {
		if(url != null) {
			...
			if (getView(viewId) != null && !url.equals(""))
				util.display(getView(viewId), url, ImageConfigHelper.getIconConfig(mContext));
		}
		return this;
	}
	...
	public ViewHolder loadImage(int viewId, String url) {
		if(url != null) {
			BitmapUtils util = new BitmapUtils(mContext, mContext.getCacheDir().getAbsolutePath());
			if (getView(viewId) != null && !url.equals(""))
				util.display(getView(viewId), url, ImageConfigHelper.getGrayConfig(mContext));
		}
		return this;
	}
	...
	public ViewHolder loadImage(int viewId, String url, BitmapLoadCallBack<View> l) {
		if(url != null) {
			...
			if (getView(viewId) != null && !TextUtils.isEmpty(url))
				util.display(getView(viewId), url, ImageConfigHelper.getGrayConfig(mContext), l);
		}
		return this;
	}
    ...
    public ViewHolder cancelBitmapUtil(int viewId) {
        BitmapUtils utils = new BitmapUtils(mContext, mContext.getCacheDir().getAbsolutePath());
        utils.cancel();
        return this;
    }
     ...
     *                  返回Holder对象可用于链式编程
	 */
	public ViewHolder setOnClickListener(int resId, OnClickListener l) {
		View view = getView(resId);
		if(view != null) view.setOnClickListener(l);
		return this;
	}

    public ViewHolder setOnLongClickListener(int resId, View.OnLongClickListener l) {
        View view = getView(resId);
        if(view != null) view.setOnLongClickListener(l);
        return this;
    }
